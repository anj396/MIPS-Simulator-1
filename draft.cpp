...
int action(){
  ...
  if(cycle_num == DRAM_curr_req.c_cycle){
      cout<<"DRAM STATUS : COLUMN ACCESSED"<<endl;
...
bool independent(DRAM_req curr_req){
  ...
  while(it != Cache_queues[j].end()){
    ...
    if(curr_req.id == 0 && temp_req.id == 0 && temp_req.to_reg == curr_req.to_reg && temp_req.to_reg != curr_req.from_reg){
      ...
      auto it1 = std::find(DRAM_queue.begin(),DRAM_queue.end(),temp_req);
      int posn = it1 - DRAM_queue.begin();
      auto it2 = helper.begin();
      advance(it2,posn);
      DRAM_queue.erase(it1);
      helper.erase(it2);
      ...
      it1 = std::find(DRAM_queue.begin(),DRAM_queue.end(),temp_req);
      int posn = it1 - DRAM_queue.begin();
      auto it2 = helper.begin();
      advance(it2,posn);
      if(it1 == DRAM_queue.end()){
        cout<<DRAM_queue.size()<<endl;
      }
      DRAM_queue.erase(it1);
      helper.erase(it2);
      it = Cache_queues[j].erase(it);
    ...
    else if(temp_req.id == 1 && curr_req.id == 0 && temp_req.from_reg == curr_req.to_reg && temp_req.Address == curr_req.Address){
      //// sw-lw such that lw is just performing exact reverse operation => lw is redundant
      curr_req.uid = -10;
      break;
    }
      ...
      curr_req.from_reg = temp_req.from_reg;
      break;
    }
...
bool independent(Instruction curr_inst, int j){
    ...
    if( op_code== 8 || op_code == 9){
      ...
      operands ops = RegFiles[j].ReadWrite(reg_num, bitset<5>(0), bitset<5>(0), word(0), bitset<1>(0));
  ...
  if(operator_temp<4){            ///// add,sub, mul
    ...
    if(curr_req.uid != -10 && curr_req.uid != -9 ){
      ...
      if(DRAM_curr_req.core_num != -1){   //// case 0 : some DRAM request is going on => push in buffer
        ...
        Cache_queues[core_num][n-1].status = curr_req.status;
      }
      else{
      ...
      bitset<5> from_reg = bitset<5>(curr_req.from_reg);
      operands ops = RegFiles[core_num].ReadWrite(from_reg, bitset<5>(0), bitset<5>(0), word(0), bitset<1>(0));
      word copy_data = ops.op1;
...
void interpret(vector<ALU>& my_ALUs, Memory& my_mem, vector<RF>& RegFiles, vector<vector<int>>& ins_counts){
  ...
  cycle_num = 1;
	while(true){
	    if(DRAM_curr_req.core_num == -1 && DRAM_queue.size()==0 && !count(done.begin(), done.end(),0)){
      ...
      for(int j = 1 ; j < N+1; j++){    /// iterating over each core
        if(done[j] == 1 && DRAM_curr_req.core_num != j && Cache_queues[j].size() == 0 ){
          ...
          if((operator_temp == 8 || operator_temp == 9) && DRAM_queue.size() >= DRAM_BUFFERSIZE){/////// include MRM here
            DRAM_alone[j] = 1;
          }
